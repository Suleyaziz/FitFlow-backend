...
class ExerciseResource(Resource):
    @token_required
    def get(self, current_user, exercise_id=None):  # Fixed: self first
        if exercise_id:
    ...
    @token_required
    def put(self, current_user, exercise_id):  # Fixed: self first
        exercise = Exercise.query.get(exercise_id)
        if not exercise:
            return {"error": "Exercise not found"}, 404
        data = request.get_json()
    ...
    @token_required
    def delete(self, current_user, exercise_id):  # Fixed: self first
        exercise = Exercise.query.get(exercise_id)
        if not exercise:
            return {"error": "Exercise not found"}, 404
